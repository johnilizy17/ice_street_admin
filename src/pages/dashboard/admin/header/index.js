...
import { adminGetAllDrivers, adminGetHeader, adminUpdateProductStatus, admimDeleteHeader } from "services/admin-services";
...
function DriverManagement() {
    ...
    const fetchDriversadminGetAllDrivers = async (e, s) => {
        try {
            setLoading(true);
            const data = await adminGetHeader();
    ...
    const DeleteProduct = async (e) => {
        try {
            setLoading(true);
            const deleting = await admimDeleteHeader(e);
            const data = await adminGetHeader(page, search);
    ...
    return (
        <Box w="100%">
            ...
            >
                <Box>
                    <Heading fontSize={["lg", "2xl"]}>Page Header Management</Heading>
                    <Text fontSize={["sm", "md"]}>Monitor Header statistics easily</Text>
                </Box>
                <ButtonGroup>
                    <a href="/dashboard/admin/header/create">
                        <Button
                            color="pink.600"
                            fontSize={["12px", "14px"]}
                                ...
                                {drivers && drivers.map((driver, index) => (
                                    ...
                                    >
                                        ...
                                        <Td fontSize="sm" onClick={() =>
                                            router.push(
                                                { pathname: `/dashboard/admin/header/${driver._id}`, query: driver }
                                            )
                                        }>
                                            {driver.title}
                                        </Td>
                                        ...
                                        <Td color="green" onClick={() => router.push({ pathname: `/dashboard/admin/header/${driver._id}`, query: driver })}>
                                            Edit
                                        </Td>
