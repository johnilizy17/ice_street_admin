...
import { adminGetAddPackage, adminGetAllDrivers, adminGetProduct } from 'services/admin-services';
...
export default function EditPackage() {
    ...
    async function AddPackage() {
        ...
        if (packageTitle.length > 0) {
            try {
                const data = await adminGetAddPackage(AddPackageData);
        Router.push("/dashboard/admin/package")
            ...
            toast({
                position: "top-right",
                title: "add package name created",
                status: "error",
                isClosable: true,
            })
        }
