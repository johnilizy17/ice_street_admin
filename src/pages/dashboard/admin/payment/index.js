...
import { adminGetAllDrivers, adminExportDriversAsCsv, adminSearchForADriver, adminGetAllPoints, adminGetAllPayment, adminGetAllOffline } from "services/admin-services";
...
function DriverManagement() {
	...
	const fetchDriversadminGetAllDrivers = async (pageNumber, type) => {
		...
		try {
			setLoading(true);
			const dataw = await adminGetAllPayment(pageNumber, type);
			setData(dataw?.data.payment)
			setLoading(false);
		} catch (error) {
	...
	return (
		<Box >
			...
			>
				<Box>
					<Heading fontSize={["lg", "2xl"]}>Payment</Heading>
					<Text fontSize={["sm", "md"]}>Monitor The payment statistics easily</Text>
				</Box>
			</Flex>
							...
							loading ?
								...
								<>
								   ...
								   <Text fontSize="14px">Show</Text>
									...
									<TabPanels>
										<TabPanel>
											<PaymentTable drivers={data} fetchDriversadminGetAllDrivers={fetchDriversadminGetAllDrivers} />
										</TabPanel>
